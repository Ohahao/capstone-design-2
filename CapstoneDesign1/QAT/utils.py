...
def separate_ensemble(ensemble, return_single=False):
    ...
    for img in ensemble:
        img = img.squeeze() #추가한 차원 제거
        if img.ndim == 3:                       # Transpose if necessary.
            img = np.transpose(img, (1, 2, 0)) #channel last 방식으로 변경

        ensemble_np.append(img) #앙상블 역변환 리스트에 추가
...
def predict_ensemble(model, ensemble, device):
    ...
    """
    y_hat_ensemble = [] #예측한 이미지를 저장할 빈 리스트 생성

    for x in ensemble:
        x = x.to(device) #받은 앙상블 속 이미지를 device로

        with torch.no_grad():
            y_hat = model(x) #앙상블 속 각 이미지에 대한 예측 수행
            y_hat_ensemble.append(y_hat.cpu().detach().numpy().astype('float32')) #예측 결과 리스트에 추가

    return y_hat_ensemble #리스트 반환
