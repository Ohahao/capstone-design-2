...
from model import RDUNet    #model로 RDUNet import
...
def predict(model, noisy_dataset, gt_dataset, device, padding, n_channels, results_path):
    ...
    X = sio.loadmat(noisy_dataset)['data'].flatten()    #test data
    Y = sio.loadmat(gt_dataset)['label'].flatten()      #정답 data
...
if __name__ == '__main__':
    ...
    #config file의 parameter 할당
    model_params = config['model']
    test_params = config['test']    #test에 필요한 pretrained model, dataset, result 등이 담겨있다.
    n_channels = model_params['channels']

    #rgb
    if n_channels == 3:
        model_path = join(test_params['pretrained models path'], 'model_color.pth')     #pretrained model path
        noisy_datasets = ['noisy_cbsd68_']  # test dataset을 넣어주는 부분 -> 여기를 바꿔야됨.
        gt_datasets = ['cbsd68_label']      # test dataset을 넣어주는 부분 -> 여기를 바꿔야됨.
