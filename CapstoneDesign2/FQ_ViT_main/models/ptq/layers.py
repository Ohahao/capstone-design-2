...
from bit_type import BIT_TYPE_DICT     #upper bound와 lower bound, range를 설정해 bit type을 다르게 하는 class이다. uint4~7이 있다.
from observer import build_observer    #observer 인스턴스를 생성한다.
from quantizer import build_quantizer  #quantizer 인스턴스를 생성한다. 

#컨볼루션 레이어에 대한 양자화를 실시하는 클래스이다.
class QConv2d(nn.Conv2d):
    ...
    def forward(self, x):
        if self.calibrate:
            #print("\n\ncalibrate - observer update 실시")
            self.quantizer.observer.update(self.weight)
            if self.last_calibrate:
                self.quantizer.update_quantization_params(x)
        if not self.quant:  #quant=False 경우 일반적인 컨볼루션 연산을 한다.
            print("\n\nquantization 안됨")
            return F.conv2d(
                x,
                self.weight,
        ...
        #quant=True인 경우 가중치를 양자화 한 후 이를 이용해 컨볼루션 연산을 실행한다. 
...
#ConvTranspose2d 레이어에 대한 양자화를 실시하는 클래스이다. 
class QConvTranspose2d(nn.ConvTranspose2d):
    def __init__(self,
        ...
        self.module_type = 'conv_transpose_weight'
        self.observer = build_observer(self.observer_str, self.module_type,
                                       self.bit_type, self.calibration_mode)
        self.quantizer = build_quantizer(self.quantizer_str, self.bit_type,
                                         self.observer, self.module_type)
    ...
    def forward(self, x, output_size=None):
        if self.calibrate:
            ...
            if self.last_calibrate:
                self.quantizer.update_quantization_params(x)    
        if not self.quant:  # quant=False 경우 일반적인 역컨볼루션 연산을 한다.
        ...
        # quant=True인 경우 가중치를 양자화 한 후 이를 이용해 역컨볼루션 연산을 실행한다.
        weight = self.quantizer(self.weight)
...
#activation에 대한 양자화를 실시하는 클래스이다.
class QAct(nn.Module):
    ...
    def __init__(self,
        ...
        self.observer_str = observer_str
        self.quantizer_str = quantizer_str
        self.actv = nn.PReLU(num_paramters)
...
#layer normalization에 대한 양자화를 실시한다. - batchnorm과 차이점: 특성 단위로 정규화 vs 데이터 샘플 단위로 정규화
