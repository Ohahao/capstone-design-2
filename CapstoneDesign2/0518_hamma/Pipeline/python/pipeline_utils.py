...
def Non_local_means_denoising(srgb_image):
    min_value = srgb_image.min()
    max_value = srgb_image.max()
    print('min: ', min_value)
    print('max: ', max_value)
    #srgb_image = srgb_image.astype(np.float32)
    srgb_image = ((srgb_image - min_value) / (max_value - min_value))
    #srgb_image = srgb_image.astype(np.uint8)
    print(srgb_image.shape)
    print(srgb_image.dtype)
    NLM_image = cv2.fastNlMeansDenoisingColored((srgb_image * 255).astype(np.uint8), None, 10, 10, 7, 21)
    NLM_image = NLM_image.astype(np.float64) / 255.0
    return NLM_image
