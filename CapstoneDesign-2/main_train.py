...
from transforms import RandomHorizontalFlip, RandomVerticalFlip, RandomRot90
...
def main():
    ...
    model_name = 'model_0402_16h' if model_params['channels'] == 3 else 'model_gray'
    ...
    with open('train_gt.txt', 'r') as f_train_gt, open('train_n.txt', 'r') as f_train_n, open('valid_gt.txt', 'r') as f_val_gt, open('valid_n.txt', 'r') as f_val_n:
        ...
        train_gt_files = list(map(lambda file: join(train_params['train_gt dataset path'], file), raw_train_gt_files))
        train_n_files = list(map(lambda file: join(train_params['train_n dataset path'], file), raw_train_n_files))
        val_gt_files = list(map(lambda file: join(val_params['valid_gt dataset path'], file), raw_val_gt_files))
        val_n_files = list(map(lambda file: join(val_params['valid_n dataset path'], file),raw_val_n_files))
