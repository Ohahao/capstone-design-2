...
def create_patches(gt_image, n_image, patch_size, step):
    gt_image = view_as_windows(gt_image, patch_size, step)
    n_image = view_as_windows(n_image, patch_size, step)
    h, w = image.shape[:2]
    gt_image = np.reshape(gt_image, (h * w, patch_size[0], patch_size[1], patch_size[2]))
    n_image = np.reshape(n_image, (h * w, patch_size[0], patch_size[1], patch_size[2]))

    return gt_image, n_image
...
class NoisyImagesDataset(Dataset):
    def __init__(self, gt_files, n_files, channels, patch_size, transform=None):
        ...
        self.load_gt_dataset(gt_files)
        self.load_n_dataset(n_files)
    ...
    def load_dataset(self, gt_files, n_files):
        ...
        for gt_file in tqdm(zip(gt_files, n_files)):
